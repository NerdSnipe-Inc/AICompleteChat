...
**Files:**
- Create: `~/Projects/AICompleteChat/project.yml`
- Create: `~/Projects/AICompleteChat/AICompleteChat.xcodeproj` (generated by `xcodegen`, not hand-authored or created via Xcode's GUI — see Step 1's ruling)
...
**Ruling (recorded because it changes how this task is executed vs. its original text):** the plan originally specified creating the Xcode project interactively via Xcode's GUI (File → New → Project). No subagent — implementer or otherwise — can drive a GUI application; this is a hard tooling constraint, not a step to work around. `xcodegen` (confirmed installed at `/opt/homebrew/bin/xcodegen`, version 2.45.3) generates an equivalent, fully valid `.xcodeproj` from a declarative `project.yml` — same end result (a buildable macOS app target wired to the six packages), fully scriptable. Steps 1–3 below replace the original GUI steps with this approach; every other step in this task is unchanged.

- [ ] **Step 1: Write project.yml and generate the Xcode project**

```yaml
# project.yml
name: AICompleteChat
options:
  bundleIdPrefix: com.nerdsnipe
  deploymentTarget:
    macOS: "15.0"
packages:
  DesignFoundation:
    path: /Users/nerdsnipe/Projects/DesignFoundation
  DesignFoundationPro:
    path: /Users/nerdsnipe/Projects/DesignFoundationPro
  AIChatKit:
    path: /Users/nerdsnipe/xCodeProjects/NerdSnipe-Inc-Packages/AIChatKit
  AIChatKitMLX:
    path: /Users/nerdsnipe/xCodeProjects/NerdSnipe-Inc-Packages/AIChatKitMLX
  AiPersona:
    path: /Users/nerdsnipe/xCodeProjects/NerdSnipe-Inc-Packages/AiPersona
  AiVoiceKit:
    path: /Users/nerdsnipe/xCodeProjects/NerdSnipe-Inc-Packages/AiVoiceKit
targets:
  AICompleteChat:
    type: application
    platform: macOS
    ...
    settings:
      base:
        PRODUCT_BUNDLE_IDENTIFIER: com.nerdsnipe.aicompletechat
        MARKETING_VERSION: "1.0"
        CURRENT_PROJECT_VERSION: "1"
        SWIFT_VERSION: "6.0"
        CODE_SIGN_STYLE: Automatic
        DEVELOPMENT_TEAM: QW2MDLZZJH
        "CODE_SIGN_IDENTITY[sdk=macosx*]": "Apple Development"
    info:
      path: AICompleteChat/Info.plist
      properties:
        CFBundleDisplayName: AICompleteChat
        LSMinimumSystemVersion: "15.0"
    dependencies:
      - package: DesignFoundation
      - package: DesignFoundationPro
      - package: AIChatKit
        product: AIChatCore
      - package: AIChatKit
        product: AIChatUI
      - package: AIChatKitMLX
        product: AIChatMLX
      - package: AiPersona
      - package: AiVoiceKit
  AICompleteChatTests:
    ...
    settings:
      base:
        SWIFT_VERSION: "6.0"
    dependencies:
      - target: AICompleteChat
```

Do **not** add `AIChatOpenAI` or `AIChatAnthropic` as packages/dependencies (Global Constraint: zero network calls for inference) — note they aren't in the spec above.

`xcodegen`'s `sources:` entries reference folders by path — create the (currently empty) source folders before generating, so the project references a real directory from the start rather than one that only appears after Step 4:

```bash
cd ~/Projects/AICompleteChat
mkdir -p AICompleteChat AICompleteChatTests
xcodegen generate
```

Expected: `Created project at ~/Projects/AICompleteChat/AICompleteChat.xcodeproj`.

- [ ] **Step 2: Verify the signing identity was applied**

Same GUI-avoidance reasoning as Step 1's ruling — signing identity is also set declaratively in `project.yml` above (`DEVELOPMENT_TEAM: QW2MDLZZJH`, matching the exact team ID already used by `alric.xcodeproj`, verified via `grep DEVELOPMENT_TEAM alric.xcodeproj/project.pbxproj` earlier this session) rather than requiring an interactive Xcode session. Confirm it landed in the generated project:

```bash
cd ~/Projects/AICompleteChat
grep -c "DEVELOPMENT_TEAM = QW2MDLZZJH" AICompleteChat.xcodeproj/project.pbxproj
```

Expected: non-zero. Do not configure a Developer ID/notarization identity yet — that's out of scope here.

- [ ] **Step 3: Verify the generated pbxproj wiring is not dangling**

Run this and confirm every product listed in Step 1's `dependencies:` has both a `PBXBuildFile` entry (`... in Frameworks`) and a matching `XCSwiftPackageProductDependency` entry — `xcodegen` should never produce the dangling-reference bug discovered and fixed by hand in `alric.xcodeproj` earlier this session (a build-file reference with no matching build-file definition), but verify rather than assume:
...
Both counts should be non-zero and every product name from Step 1 should appear in both.
...
**Verified against real DesignFoundation source before writing this** (unlike earlier tasks' brief text, which guessed wrong on `DFText`/`DFEmptyState`/`DFBanner`/`DFIcon` and had to be corrected mid-dispatch — Tasks 1 and 8 both hit this): `DFText`'s real init is `DFText(_ content: String, scale: DFTextScale = .body)` (not `role:`); `DFEmptyState`'s real init is `DFEmptyState(icon: String, title: String, message: String? = nil, actionTitle: String? = nil, onAction: (...)? = nil, secondaryActionTitle: String? = nil, onSecondaryAction: (...)? = nil)` (not `systemImage:`); `DFIcon`'s real init is `DFIcon(_ symbolName: String, size: CGFloat? = nil)` (no `systemName:` label, no `color:` param — wrap in `.foregroundStyle(...)` instead); `theme.colors` has no `.error` token, the real one is `.destructive`; `DFBanner`'s real init is `DFBanner(icon: String? = nil, message: String, severity: DFToastSeverity = .info, actionTitle: String? = nil, onAction: (...)? = nil, isDismissible: Bool = false, onDismiss: (...)? = nil)` (no `title:`, no `style:`, no nested `action:` struct — flat `actionTitle:`/`onAction:`); `DFButton(_:style:role:action:)` with `style: .outlined` IS correct as originally written, confirmed against `DFButtonStyle.swift`'s `static var outlined`.

```swift
public var body: some View {
    switch configuration.loadState {
    case .notLoaded:
        DFEmptyState(icon: "cpu", title: "Preparing on-device model", message: "This starts automatically.")
    case .downloading(let progress):
        VStack(spacing: theme.spacing.md) {
            DFText("Downloading Gemma 4 e4b…", scale: .headline)
            ProgressView(value: progress)
                .frame(maxWidth: 320)
            DFText("\(Int(progress * 100))%", scale: .caption)
        }
    case .error(let message):
        VStack(spacing: theme.spacing.md) {
            DFIcon("exclamationmark.triangle")
                .foregroundStyle(theme.colors.destructive)
            DFText("Model failed to load", scale: .headline)
            DFText(message, scale: .body)
...
if !configuration.micPermissionGranted {
    DFBanner(
        ...
        onAction: configuration.onOpenSystemSettings
    )
}
...
`DFToastSeverity` (`Sources/DesignFoundation/Supplementary/Toast/DFToastStyle.swift`) has cases `info`/`success`/`warning`/`error` — `.warning` above is confirmed correct.
...
- [ ] **Step 2b: Add a minimal no-op ChatProvider for ContentView's placeholder session**

`ContentView.init()` needs *some* `ChatSession` to satisfy `@ObservedObject`'s property wrapper before `@Environment` is readable (same reasoning as the `voiceEngine` placeholder) — it's discarded immediately in `.task` below and never actually used to generate anything. Add directly to `ContentView.swift` (marked `private` — it's file-scoped and only ever used in `init()` below, in the same file):

```swift
/// Placeholder-only ChatProvider, used solely to satisfy ContentView's @ObservedObject
/// property wrapper before @Environment is readable in init(). Never actually streams —
/// the real session (appEnvironment.session, backed by MLXProvider) replaces this in .task.
...
- [ ] **Step 3: Wire ContentView to the redesigned root view + settings sheet, with real voice/memory/chat reactivity**

Two things `ChatSession`/`VoiceEngineMacOS` share: both are Combine `ObservableObject`s (`@Published` properties), not `@Observable` types — reading either only through the `@Observable AppEnvironment` wrapper does **not** make SwiftUI re-render when their `@Published` properties change, because Observation tracks access to `@Observable`-macro'd properties, not a nested object's own `@Published` changes. `ContentView` must hold its own `@ObservedObject` reference to both `appEnvironment.voiceEngine` and `appEnvironment.session` for those specific subscriptions.

Separately: `DFAIChatThreadScreen`/`DFAIChatNewScreen` render `AIChatConversation`/`AIChatMessage` (DesignFoundationPro's value types) — they know nothing about `ChatSession.Entry` (AIChatUI's streaming display model). Nothing bridges these automatically; `ContentView` is where that conversion belongs, since it's the one layer that imports both `DesignFoundationPro` and `AIChatUI`.
...
struct ContentView: View {
    ...
    init() {
        // Placeholder engine/session for the property initializers; replaced immediately in .task
        // below once appEnvironment is available — SwiftUI doesn't allow reading @Environment inside init.
        self._voiceEngine = ObservedObject(wrappedValue: VoiceEngineMacOS(onCommandReceived: { _ in }))
        self._session = ObservedObject(wrappedValue: ChatSession(provider: NoopChatProvider(), model: ""))
    }
    ...
    var body: some View {
        ...
        .task {
            // Re-point @ObservedObject at the real, shared instances from AppEnvironment now
